...
const { TicketImg, Ticket, User } = require(`./../database/database`);
...
module.exports = {
  ...
  sendAfterPayment: async (req, res, next) => {
    ...
    const foundTickets = await TicketImg.findAll({ where: { id: ticketsId } });
    ...
    if (foundTickets) {
      ...
      return await whatsappSend(phoneNumber, msgBody, req, res, async () => {
        ...
        const buyer = User.findOne({ where: { phoneNumber: phoneNumber } });
        if (buyer) {
        ...
        const sellerIsRegistered = await client.isRegisteredUser(
          `${sellerPhoneNumber}@c.us`
        );
        if (sellerPhoneNumber.length > 10 && sellerIsRegistered) {
